...
func BenchmarkPricedListDiscardTrueDropsLenTxs(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		slots := int(txsCount)
		b.StartTimer()
		drops := list.Discard(slots, accounts)
		b.StopTimer()
...
func BenchmarkPricedListDiscardFalseDropsLenTxsMinus1(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		drops := list.Discard(slots, accounts)
		b.StopTimer()
		assert.Equal(b, int(slots-1), len(drops)) // drops are got: heap.Pop(items)
		assert.Equal(b, -1, list.stales)          // updated since not to get from all once: stales.Add(-1)
		assert.Equal(b, 0, len(*list.items))      // updated since heap.Pop(items)
		assert.Equal(b, int(txsCount-1), len(list.all.txs))
		list.Removed()
	}
}
